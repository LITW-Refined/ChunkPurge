...
/*
 * A class to handle the unloading of excess chunks from a WorldServer. Excess loaded chunks are those that are
 * currently not within a player's view distance, forced by a chunk loader, or loaded by the world's spawn area.
 */
public class WorldChunkUnloader {
    ...
    /*
     * A flood fill algorithm to find the shape of the loaded chunks surrounding a player-occupied chunk, or seed. Will
     * not return chunks that are further than radiusLimit from the seed. Set radiusLimit to 0 in order to ignore any
     * limit. 1. Set Q to the empty queue. 2. If the color of node is not equal to target-color, return. 3. Add node to
     * Q. 4. For each element N of Q: 5. If the color of N is equal to target-color: 6. Set w and e equal to N. 7. Move
     * w to the west until the color of the node to the west of w no longer matches target-color. 8. Move e to the east
     * until the color of the node to the east of e no longer matches target-color. 9. For each node n between w and e:
     * 10. Set the color of n to replacement-color. 11. If the color of the node to the north of n is target-color, add
     * that node to Q. 12. If the color of the node to the south of n is target-color, add that node to Q. 13. Continue
     * looping until Q is exhausted. 14. Return
     */
    ...
    /*
     * Populate chunksToUnload with chunks that are isolated from all players, chunk loaders, and the spawn. Use a
     * flood-fill algorithm to find the set of all loaded chunks in the world which link back to a chunk watcher through
     * other loaded chunks. The idea is to find the isolated chunks which do NOT link back to a valid chunk watcher, and
     * unload those. This is a better alternative to simply unloading all chunks outside of a player's view radius.
     * Unloading chunks while not unloading their neighbors would result in tps-spikes due to the breaking of energy
     * nets and the like. This approach should reduce the severity of those tps-spikes. ChunkProviderServer.loadedChunks
     * is a private field, so an access transformer is required to access it.
     */
    ...
    /*
     * Analyze the chunks that are currently loaded in this world. Select loaded chunks that are isolated from any chunk
     * watchers, and queue these isolated chunks for unloading.
     */
    ...
    public void unloadChunks() {
        ...
        if (this.world.getChunkProvider() instanceof ChunkProviderServer) {
            ...
            for (ChunkCoordIntPair coord : chunksToUnload) {
                ...
                ((ChunkProviderServer) this.world.getChunkProvider())
                        .unloadChunksIfNotNearSpawn(coord.chunkXPos, coord.chunkZPos);
            }

        }

        if (ModChunkPurge.config.debug) {
            ModChunkPurge.log.log(
                    Level.INFO,
                    "Queued " + String.valueOf(chunksToUnload.size())
                            + " chunks for unload in dimension "
                            + this.world.provider.getDimensionName()
                            + " ("
                            + String.valueOf(this.world.provider.dimensionId)
                            + ") in "
                            + String.valueOf(MinecraftServer.getSystemTimeMillis() - this.initialTime)
                            + " milliseconds.");
